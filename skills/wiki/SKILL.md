...
description: >
  ...
  Wiki pages synthesize knowledge from artifacts, bib entries, xettel cards,
...
- Use Qdrant semantic search with `-t` filter to find related content when creating or updating a page. This surfaces connections that MOC navigation might miss.
  ```
  thinkbox/scripts/search.sh '<topic>' -t bib    # sources to cite
  thinkbox/scripts/search.sh '<topic>' -t x      # user's thoughts to reference
  thinkbox/scripts/search.sh '<topic>' -t wiki   # existing pages to link or update
  thinkbox/scripts/search.sh '<topic>'            # all types — for broad discovery
  ```
...
A wiki page with a `twitter_id` is **published and stable.** Bridge cards and tweets may reference it. Content can be enriched (new sources, additional detail), but must not be rewritten to contradict its original position. If new information contradicts a published page, create a new page with the new position and note the contradiction on both.

## Relationship to other content

- Wiki reads artifacts (via bib) — the primary knowledge source
- Wiki references xettel cards — treats user's thoughts as first-class sources
- Wiki does NOT copy xettel text — it links and synthesizes
